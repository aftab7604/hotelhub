                        ...
                        <div class="row col-print-row">
							<?php $items = admin_helper::get_subcat_active_edit_items($category->c_id);$i=0;
                                if(is_array($items)){foreach($items as $item){$i++;
                                    ...
                                    <div class="col-lg-4 col-xs-12 col-sm-12 col-print-4">
                                ...
                                if(is_array($items)){foreach($items as $item){$i++;
                                    ...
                                    <tr>
                                        <td>
                                         <div class="checkbox checkbox-success">
                                             <input data-toggle="modal" data-target="add_item" type="checkbox" <?php if(count($items_checked) > 0){echo 'checked="checked"';}?> class="checkbox_<?php echo $category->c_id;?>" name="<?php echo $category->c_id;?>" value="<?php echo $item->s_id;?>" id="subCat_<?php echo $item->s_id;?>">
                                             <label> <?php echo $item->subcat_name;?></label>
                                         </div>   
                                        </td>
                                        <td>
                                            <?php if(count($items_checked) > 0){if($items_checked[0]->flag == 'flag'){
                                                echo 'Flagged: '.$items_checked[0]->flag_type;
                                            }}?>
                                        </td>
                                        <td>
                                            <?php if(count($items_checked) > 0){if($items_checked[0]->filename != ''){?>
                                                <img width="50" src="<?php echo base_url();?>assets/images/pmp_images/<?php echo $items_checked[0]->filename;?>"/>
                                            <?php }}?>
                                        </td>
                                        <td>
                                            <?php if(count($items_checked) > 0){if($items_checked[0]->notes != ''){
                                                echo 'Notes: '.htmlspecialchars_decode($items_checked[0]->notes);
                                            }}?>
                                        </td>
                                    </tr>    
                            <?php }}?>
